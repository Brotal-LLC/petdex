...
fn bubbleView(ui: *AppUi, model: *const Model) AppUi.Node {
    ...
    if (builtin.target.os.tag != .macos) {
        ...
        const mode = bubbleDisplayMode(model);
        const disclosure_presentation = bubbleDisclosurePresentation(model);
        // The disclosure's mode changes often, but it is one keyboard
        // control. Keeping a single keyed button kind retains its canvas
        // identity (and therefore focus) through All → Recent → Hidden → All.
        // The visible icon/count remains mode-specific; only the underlying
        // focus target is stable.
        ...
        const control_icon = switch (mode) {
            .all => "app:bubble-layers",
            .recent => "app:bubble-recent",
            .hidden => if (disclosure_presentation.show_status_icon) status_icon else "",
        };
        const control_label = switch (mode) {
            .all => "Show most recent active session only",
            .recent => "Hide session cards",
            .hidden => "Show all active sessions",
        };
        const count_text = if (mode == .hidden and !disclosure_presentation.show_status_icon)
            std.fmt.bufPrint(&bubble_disclosure_count_scratch, "{d}", .{model.bubbles_len}) catch ""
        else
            "";
        var control = ui.button(.{
            .global_key = .{ .str = "petdex-bubble-disclosure" },
            .width = disclosure.w,
            .height = disclosure.h,
            .size = .sm,
            .variant = .ghost,
